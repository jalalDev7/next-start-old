...
import { checkDisponibilte, updateReservation } from "@/actions/reservation";
...
const ReservationEditForm = ({
  ...
  const [coupon, setCoupon] = useState<string | undefined>(
    reservation.couponId || undefined
  );
  ...
  const [endDate, setEndDate] = useState<Date | undefined>(reservation.endDate);
  const [periodDone, setPeriodDone] = useState(true);
  const form = useForm<z.infer<typeof reservationSchema>>({
    resolver: zodResolver(reservationSchema),
    defaultValues: {
      vehiculeId: vehicule.id,
      clientId: client.id,
      assurance: reservation.assurance || undefined,
      couponId: reservation.couponId || undefined,
      dropOff: reservation.dropOff || undefined,
      dropOffTime: reservation.dropOffTime || undefined,
      endDate: endDate,
      startDate: startDate,
      limitKm: reservation.limitKm || undefined,
      note: reservation.note || undefined,
      paymentStatut: reservation.paymentStatut,
      pickup: reservation.pickup || undefined,
      pickupTime: reservation.pickupTime || undefined,
  ...
  const onSubmit = (values: z.infer<typeof reservationSchema>) => {
    if (periodDone) {
      startTransition(() => {
        updateReservation(values, reservation.id).then((res) => {
          if (res.error) {
      ...
      <div className="flex flex-col w-full bg-secondary border border-primary/15 p-4 mt-4">
        <div className="flex flex-row items-start justify-between w-full pb-4 border-b border-primary/15">
          <div className="flex flex-col gap-1 w-full">
            ...
            {client.permis ? (
              <h3 className="text-sm">
                <span className="font-semibold">Identifiant de permis : </span>
                {client.permis}
              </h3>
            ) : null}
            {client.passport ? (
          ...
          <div className="flex flex-col w-full justify-end ">
            <div className="flex flex-col gap-1 w-full items-end">
              ...
              <div className="flex flex-row items-start  justify-between gap-4 text-nowrap">
